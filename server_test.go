...
func TestSort(t *testing.T) {
	...
	team := []Player{
		...
		{Username: "alice", Score: 34},
		{Username: "sue", Score: 874},
	}
	...
	if team[0].Username != "sue" {
		t.Fatalf("expected sue to be first, got: %s", team[0].Username)
	}
	if team[3].Username != "bob" {
		t.Fatalf("expected bob to be last, got: %s", team[3].Username)
	}
}
